...
const Profile = () => {
  ...
  // ✅ Clear persisted Redux + localStorage
  const clearPersistHandler = () => {
    persister.purge();
    dispatch(signOut());
    alert("Persist cleared. Reloading...");
    ...
    <div className='max-w-xl mx-auto  bg-gray-100 min-h-screen'>
      <div className='max-w-xl mx-auto p-5 bg-white shadow-md rounded-lg mt-10'>
        ...
        <form onSubmit={formSubmitHandler} className='flex flex-col m-5 gap-4'>
          ...
          <input type="text" placeholder='fullName' className='border-1 active:border-2 p-3 rounded-lg' id="fullName" onChange={ChangeHandler} />
          <input type="text" placeholder='email' className='border-1 active:border-2 p-3 rounded-lg' id="email" onChange={ChangeHandler} />
          <input type="text" placeholder='username' className='border-1 active:border-2 p-3 rounded-lg' id="username" onChange={ChangeHandler} />
      ...
      {/* ✅ Buttons: Delete, Sign Out, Clear Persist */}
      <div className='flex flex-col sm:flex-row justify-between items-center mt-1 gap-3 px-4'>
        ...
        <button className='bg-yellow-500 p-2 rounded-lg text-white hover:opacity-95' onClick={clearPersistHandler}>
          Clear Session (Fix Broken Login)
        </button>
      </div>

      <input type="file" accept="image/*" onChange={(e) => setFile(e.target.files[0])} ref={imgref} hidden />
      <button className='w-full text-center text-green-400 cursor-pointer mt-3' onClick={() => setListing((prev) => !prev)}>Show Listings</button>
          ...
          {list.map((listing) => (
            <div key={listing._id} className='border rounded-lg p-3 flex justify-between items-center gap-4 '>
              ...
              <div className='flex flex-col item-center justify-center'>
                <button onClick={() => DeleteHandler(listing)} className='bg-red-500  rounded-lg p-1 text-sm text-white'>
                  Delete
                </button>
